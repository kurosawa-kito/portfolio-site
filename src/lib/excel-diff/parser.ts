...
import { ExcelFile, ExcelSheet, ExcelRow, ExcelCell, CellValue } from '@/types/excel';
...
export function exportToExcel(excelData: ExcelFile): Blob {
  ...
  excelData.sheets.forEach(sheet => {
    const wsData = sheet.rows.map(row => 
 ...
 * 共通のシート名を取得する
 */
export function getCommonSheetNames(file1: ExcelFile, file2: ExcelFile): string[] {
