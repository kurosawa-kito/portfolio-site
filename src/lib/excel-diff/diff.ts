 ...
 * 行レベルの差分を計算する
 */
function compareRows(originalSheet: ExcelSheet, modifiedSheet: ExcelSheet): RowDiff[] {
 ...
 * セルレベルの差分を計算する
 */
function compareCells(originalRow: ExcelRow, modifiedRow: ExcelRow): CellDiff[] {
  const result: CellDiff[] = [];
  const maxCells = Math.max(originalRow.cells.length, modifiedRow.cells.length);
  
  for (let i = 0; i < maxCells; i++) {
    const originalCell = i < originalRow.cells.length ? originalRow.cells[i].value : null;
    const modifiedCell = i < modifiedRow.cells.length ? modifiedRow.cells[i].value : null;
 ...
 * インデックスから差分行を取得する
 */
export function getDiffAtIndex(diffResults: DiffResult[], index: number): { diffResult: DiffResult, rowDiff: RowDiff } | null {
  let counter = 0;
