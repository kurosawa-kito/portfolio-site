...
const DiffNavigation: React.FC = () => {
  ...
  const totalDiffs = diffResults.reduce(
    (total, result) => total + result.rowDiffs.filter(row => row.type !== 'unchanged').length,
    0
  );
  ...
  return (
    ...
    >
      <HStack spacing={4} justify="space-between" align="center">
        <HStack spacing={3}>
          <Text fontSize="sm" fontWeight="medium">差分：</Text>
          ...
          {currentDiffType && (
            <>
              <Badge colorScheme={getBadgeColor()}>
                {getDiffTypeLabel()}
              </Badge>
        ...
        <HStack spacing={2}>
          ...
          <Tooltip label="次の差分 (↓)">
            <Button
              ...
              isDisabled={currentDiffIndex >= totalDiffs - 1 || totalDiffs === 0}
            >
              次へ
            </Button>
          </Tooltip>
