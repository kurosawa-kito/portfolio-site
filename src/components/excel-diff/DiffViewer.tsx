...
const DiffViewer: React.FC<DiffViewerProps> = ({ sheetName }) => {
  ...
  // スプレッドシートの差分表示
  return (
    <Box overflowX="auto" overflowY="auto" maxH="600px" borderWidth="1px" borderRadius="md">
                ...
                <Td>
                  <HStack spacing={2}>
                    ...
                    {rowDiff.type !== 'unchanged' && (
                      <Badge 
                        colorScheme={
                          rowDiff.type === 'added' ? 'green' : 
                          rowDiff.type === 'removed' ? 'red' : 'yellow'
                        }
                      >
                        {rowDiff.type === 'added' ? '追加' : 
                         rowDiff.type === 'removed' ? '削除' : '変更'}
                      </Badge>
                    )}
                  </HStack>
                </Td>
                
                {rowDiff.cellDiffs && rowDiff.cellDiffs.map((cellDiff, cellIdx) => {
                  // セルの状態に応じた背景色の設定
                  let cellBg;
                  if (cellDiff.type === 'added') cellBg = addedCellBg;
                  else if (cellDiff.type === 'removed') cellBg = removedCellBg;
                  else if (cellDiff.type === 'modified') cellBg = modifiedCellBg;
